        ...
        foreach ($fields as $value) {
            if (!Str::contains($value, '.')) {
                if (!in_array($value, ['image', 'photo', 'profile', 'banner'])) {
                    ...
                    $str .= '<td><a target="_blank" href="{{ asset($' . $modelName . '->' . $value . ') }}"><img class="rounded-circle img-fluid" width="50" height="50" src="{{ asset($' . $modelName . '->' . $value . ') }}" alt="' . $value . '"></a></td>' . $this->makeTab(1,
                            end($fields) != $value);
                }
            } else {
                $newValue = $this->parseDots($value);
                $str .= '<td> {{ $' . $modelName . '->' . $newValue . ' }} </td>' . $this->makeTab(1,
                        end($fields) != $value);
            }
        }
    ...
    public function parseTitlesInBlade()
    {
        ...
        foreach ($fields as $field) {
            if (!Str::contains($field, '.')) {
                $sortName = $field;
                $field = ucfirst($field);
                $str .= "<td style='cursor: pointer' wire:click=\"sort('$sortName')\"> <i class='fa @if(" . '$sortType' . " == 'desc' and " . '$sortColumn' . " == '$sortName') fa-sort-amount-down ml-2 @elseif(" . '$sortType == ' . "'asc' and " . '$sortColumn' . " == '$sortName') fa-sort-amount-up ml-2 @endif'></i> {{ __('$field') }} </td>" . $this->makeTab(6,
                        end($fields) != $field);
            } else {
