<tr x-data="{ deleteModal : false, createModal : false }">
    ...
    <td> @if($crud->active) <span class="badge badge-success">Active</span> @else <span class="badge badge-warning">Inactive</span> @endif
    </td>
    <td> @if($crud->built) <span class="badge badge-primary">Yes</span> @else <span
    ...
    <td>
        ...
        <div x-show="deleteModal" class="cs-modal animate__animated animate__fadeIn">
            <div class="bg-white shadow rounded p-5" @click.away="deleteModal = false">
                ...
                <div class="mt-5 d-flex justify-content-between">
                    ...
                    <a @click.prevent="deleteModal = false"
        ...
        <div x-show="createModal" class="cs-modal animate__animated animate__fadeIn">
            <div class="bg-white shadow rounded p-5" @click.away="createModal = false">
                ...
                <p>{{ __('BuildMessage') }}</p>
                <div class="mt-5 d-flex justify-content-between">
                    <a @click.prevent="createModal = false" wire:click.prevent="build"
                       class="text-white btn btn-success shadow">{{ __('Yes, Build it') }}</a>
                    <a @click.prevent="createModal = false"
                       class="text-white btn btn-danger shadow">{{ __('No, Cancel it.') }}</a>
                </div>
            </div>
        </div>
        ...
        @else
            <button wire:click.prevent="active" class="btn text-success mt-1" title="Active CRUD">
